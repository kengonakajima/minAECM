...
class LogLineRef {
 public:
  ...
  web_rtc::Timestamp timestamp() const { return timestamp_; }
 ...
 private:
  ...
  void set_timestamp(web_rtc::Timestamp timestamp) { timestamp_ = timestamp; }
  ...
  web_rtc::Timestamp timestamp_ = web_rtc::Timestamp::MinusInfinity();
...
namespace web_rtc_logging_impl {
...
#define RTC_LOG_FILE_LINE(sev, file, line)        \
  ::rtc::web_rtc_logging_impl::LogCall() &         \
      ::rtc::web_rtc_logging_impl::LogStreamer<>() \
          << ::rtc::web_rtc_logging_impl::LogMetadata(file, line, sev)
...
#define RTC_LOG_E(sev, ctx, err)                                 \
  !rtc::LogMessage::IsNoop<::rtc::sev>() &&                      \
      ::rtc::web_rtc_logging_impl::LogCall() &                    \
          ::rtc::web_rtc_logging_impl::LogStreamer<>()            \
              << ::rtc::web_rtc_logging_impl::LogMetadataErr {    \
...
#define RTC_LOG_TAG(sev, tag)                                 \
  !rtc::LogMessage::IsNoop(sev) &&                            \
      ::rtc::web_rtc_logging_impl::LogCall() &                 \
          ::rtc::web_rtc_logging_impl::LogStreamer<>()         \
              << ::rtc::web_rtc_logging_impl::LogMetadataTag { \
    sev, ::rtc::web_rtc_logging_impl::AdaptString(tag)         \
  }
...
#define RTC_DLOG_EAT_STREAM_PARAMS()                \
  while (false)                                     \
  ::rtc::web_rtc_logging_impl::LogMessageVoidify() & \
      (::rtc::web_rtc_logging_impl::LogStreamer<>())
