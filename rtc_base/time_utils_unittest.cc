...
TEST(FakeClock, TimeFunctionsUseFakeClock) {
  ...
  clock.SetTime(web_rtc::Timestamp::Micros(987654));
...
TEST(FakeClock, SetTime) {
  FakeClock clock;
  clock.SetTime(web_rtc::Timestamp::Micros(123));
  EXPECT_EQ(123000, clock.TimeNanos());
  clock.SetTime(web_rtc::Timestamp::Micros(456));
  EXPECT_EQ(456000, clock.TimeNanos());
}

TEST(FakeClock, AdvanceTime) {
  FakeClock clock;
  clock.AdvanceTime(web_rtc::TimeDelta::Micros(1u));
  EXPECT_EQ(1000, clock.TimeNanos());
  clock.AdvanceTime(web_rtc::TimeDelta::Micros(2222u));
  EXPECT_EQ(2223000, clock.TimeNanos());
  clock.AdvanceTime(web_rtc::TimeDelta::Millis(3333u));
  EXPECT_EQ(3335223000, clock.TimeNanos());
  clock.AdvanceTime(web_rtc::TimeDelta::Seconds(4444u));
...
TEST(FakeClock, SettingTimeWakesThreads) {
  ...
  clock.AdvanceTime(web_rtc::TimeDelta::Seconds(60u));
  EXPECT_TRUE(message_handler_dispatched.Wait(web_rtc::TimeDelta::Zero()));
  worker->Stop();
