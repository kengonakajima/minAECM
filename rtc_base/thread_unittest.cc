...
TEST(ThreadPostDelayedTaskTest, IsCurrentTaskQueue) {
  auto current_tq = web_rtc::TaskQueueBase::Current();
  {
    std::unique_ptr<rtc::Thread> thread(rtc::Thread::Create());
    thread->WrapCurrent();
    EXPECT_EQ(web_rtc::TaskQueueBase::Current(),
              static_cast<web_rtc::TaskQueueBase*>(thread.get()));
    thread->UnwrapCurrent();
  }
  EXPECT_EQ(web_rtc::TaskQueueBase::Current(), current_tq);
}

class ThreadFactory : public web_rtc::TaskQueueFactory {
 public:
  std::unique_ptr<web_rtc::TaskQueueBase, web_rtc::TaskQueueDeleter>
  CreateTaskQueue(absl::string_view /* name */,
                  Priority /*priority*/) const override {
    std::unique_ptr<Thread> thread = Thread::Create();
    thread->Start();
    return std::unique_ptr<web_rtc::TaskQueueBase, web_rtc::TaskQueueDeleter>(
        thread.release());
  }
};

std::unique_ptr<web_rtc::TaskQueueFactory> CreateDefaultThreadFactory(
    const web_rtc::FieldTrialsView*) {
  return std::make_unique<ThreadFactory>();
}

using ::web_rtc::TaskQueueTest;
