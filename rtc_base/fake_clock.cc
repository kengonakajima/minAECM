...
int64_t FakeClock::TimeNanos() const {
  web_rtc::MutexLock lock(&lock_);
  return time_ns_;
}

void FakeClock::SetTime(web_rtc::Timestamp new_time) {
  web_rtc::MutexLock lock(&lock_);
  RTC_DCHECK(new_time.us() * 1000 >= time_ns_);
  time_ns_ = new_time.us() * 1000;
}

void FakeClock::AdvanceTime(web_rtc::TimeDelta delta) {
  web_rtc::MutexLock lock(&lock_);
  time_ns_ += delta.ns();
}

void ThreadProcessingFakeClock::SetTime(web_rtc::Timestamp time) {
  clock_.SetTime(time);
...
void ThreadProcessingFakeClock::AdvanceTime(web_rtc::TimeDelta delta) {
