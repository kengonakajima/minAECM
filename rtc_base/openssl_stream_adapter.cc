...
bool ShouldAllowLegacyTLSProtocols() {
  return g_use_legacy_tls_protocols_override.load()
             ...
             : web_rtc::field_trial::IsEnabled("WebRTC-LegacyTlsProtocols");
}
...
void OpenSSLStreamAdapter::SetTimeout(int delay_ms) {
  ...
  timeout_task_ = web_rtc::RepeatingTaskHandle::DelayedStart(
      owner_, web_rtc::TimeDelta::Millis(delay_ms),
      [flag = task_safety_.flag(), this]() {
        if (flag->alive()) {
          ...
          if (res > 0) {
            ...
            return web_rtc::TimeDelta::PlusInfinity();
          }
          ContinueSSL();
        } else {
