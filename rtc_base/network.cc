...
namespace rtc {
namespace {
using ::web_rtc::SafeTask;
using ::web_rtc::TimeDelta;
...
bool PreferGlobalIPv6Address(const web_rtc::FieldTrialsView* field_trials) {
  ...
  if (field_trials &&
    ...
    web_rtc::FieldTrialParameter<bool> prefer_global_ipv6_address_enabled(
        "PreferGlobalIPv6Address", false);
    web_rtc::ParseFieldTrial(
...
web_rtc::MdnsResponderInterface* NetworkManager::GetMdnsResponder() const {
  return nullptr;
}

NetworkManagerBase::NetworkManagerBase(
    const web_rtc::FieldTrialsView* field_trials)
...
void BasicNetworkManager::StartUpdating() {
  ...
  if (start_count_) {
    ...
    task_safety_flag_ = web_rtc::PendingTaskSafetyFlag::Create();
    thread_->PostTask(SafeTask(task_safety_flag_, [this] {
      RTC_DCHECK_RUN_ON(thread_);
...
web_rtc::MdnsResponderInterface* Network::GetMdnsResponder() const {
...
uint16_t Network::GetCost(const web_rtc::FieldTrialsView* field_trials) const {
  return GetCost(
      *web_rtc::AlwaysValidPointer<const web_rtc::FieldTrialsView,
                                  web_rtc::FieldTrialBasedConfig>(field_trials));
}
