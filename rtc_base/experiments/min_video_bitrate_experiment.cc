...
absl::optional<int> GetFallbackMinBpsFromFieldTrial(VideoCodecType type) {
  ...
  if (!web_rtc::field_trial::IsEnabled(kForcedFallbackFieldTrial)) {
    return absl::nullopt;
  }

  const std::string group =
      web_rtc::field_trial::FindFullName(kForcedFallbackFieldTrial);
  if (group.empty()) {
    return absl::nullopt;
  }
...
absl::optional<DataRate> GetExperimentalMinVideoBitrate(VideoCodecType type) {
  ...
  if (web_rtc::field_trial::IsEnabled(kMinVideoBitrateExperiment)) {
    web_rtc::FieldTrialFlag enabled("Enabled");

    // Backwards-compatibility with an old experiment - a generic minimum which,
    // if set, applies to all codecs.
    web_rtc::FieldTrialOptional<web_rtc::DataRate> min_video_bitrate("br");

    // New experiment - per-codec minimum bitrate.
    web_rtc::FieldTrialOptional<web_rtc::DataRate> min_bitrate_vp8("vp8_br");
    web_rtc::FieldTrialOptional<web_rtc::DataRate> min_bitrate_vp9("vp9_br");
    web_rtc::FieldTrialOptional<web_rtc::DataRate> min_bitrate_av1("av1_br");
    web_rtc::FieldTrialOptional<web_rtc::DataRate> min_bitrate_h264("h264_br");

    web_rtc::ParseFieldTrial(
        {&enabled, &min_video_bitrate, &min_bitrate_vp8, &min_bitrate_vp9,
         &min_bitrate_av1, &min_bitrate_h264},
        web_rtc::field_trial::FindFullName(kMinVideoBitrateExperiment));
