...
#if defined(WEBRTC_IOS)
  return web_rtc::field_trial::IsEnabled(kFieldTrial);
#else
  return !web_rtc::field_trial::IsDisabled(kFieldTrial);
#endif
}

absl::optional<QualityScalingExperiment::Settings>
QualityScalingExperiment::ParseSettings() {
  std::string group = web_rtc::field_trial::FindFullName(kFieldTrial);
