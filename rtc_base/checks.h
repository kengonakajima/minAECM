...
#define RTC_EAT_STREAM_PARAMETERS(ignored)                          \
  (true ? true : ((void)(ignored), true))                           \
      ? static_cast<void>(0)                                        \
      : ::rtc::web_rtc_checks_impl::FatalLogCall<false>("", 0, "") & \
            ::rtc::web_rtc_checks_impl::LogStreamer<>()
...
#define RTC_CHECK(condition)                                    \
  (condition) ? static_cast<void>(0)                            \
              : ::rtc::web_rtc_checks_impl::FatalLogCall<false>( \
...
#define RTC_CHECK_OP(name, op, val1, val2)                 \
  ::rtc::Safe##name((val1), (val2))                        \
      ? static_cast<void>(0)                               \
      : ::rtc::web_rtc_checks_impl::FatalLogCall<true>(     \
            __FILE__, __LINE__, #val1 " " #op " " #val2) & \
            ::rtc::web_rtc_checks_impl::LogStreamer<>() << (val1) << (val2)
#else
#define RTC_CHECK(condition)                                                  \
  (condition)                                                                 \
      ? static_cast<void>(0)                                                  \
      : true ? ::rtc::web_rtc_checks_impl::FatalLogCall<false>(__FILE__,       \
                                                              __LINE__, "") & \
                   ::rtc::web_rtc_checks_impl::LogStreamer<>()                 \
             : ::rtc::web_rtc_checks_impl::FatalLogCall<false>("", 0, "") &    \
                   ::rtc::web_rtc_checks_impl::LogStreamer<>()
...
#define RTC_CHECK_OP(name, op, val1, val2)                                   \
  ::rtc::Safe##name((val1), (val2))                                          \
      ? static_cast<void>(0)                                                 \
      : true ? ::rtc::web_rtc_checks_impl::FatalLogCall<true>(__FILE__,       \
                                                             __LINE__, "") & \
                   ::rtc::web_rtc_checks_impl::LogStreamer<>()                \
             : ::rtc::web_rtc_checks_impl::FatalLogCall<false>("", 0, "") &   \
                   ::rtc::web_rtc_checks_impl::LogStreamer<>()
...
#define RTC_CHECK_NOTREACHED()                         \
  do {                                                 \
    ::rtc::web_rtc_checks_impl::UnreachableCodeReached( \
        RTC_UNREACHABLE_FILE_AND_LINE_CALL_ARGS);      \
  } while (0)

#define RTC_FATAL()                                                  \
  ::rtc::web_rtc_checks_impl::FatalLogCall<false>(__FILE__, __LINE__, \
                                                 "FATAL()") &        \
      ::rtc::web_rtc_checks_impl::LogStreamer<>()
