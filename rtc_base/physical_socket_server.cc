...
int PhysicalSocketServer::ToCmsWait(web_rtc::TimeDelta max_wait_duration) {
  return max_wait_duration == Event::kForever
             ? kForeverMs
             : max_wait_duration.RoundUpTo(web_rtc::TimeDelta::Millis(1)).ms();
}

#if defined(WEBRTC_POSIX)

bool PhysicalSocketServer::Wait(web_rtc::TimeDelta max_wait_duration,
                                bool process_io) {
