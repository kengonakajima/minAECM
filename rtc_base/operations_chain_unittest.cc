...
TEST(OperationsChainTest, AsynchronousOperation) {
  ...
  EXPECT_FALSE(
      async_operation_completed_event->Wait(web_rtc::TimeDelta::Zero()));
...
TEST(OperationsChainTest,
  ...
  EXPECT_TRUE(event0.Wait(web_rtc::TimeDelta::Zero()));
  ...
  EXPECT_TRUE(event1.Wait(web_rtc::TimeDelta::Zero()));
}
...
TEST(OperationsChainTest, AsynchronousOperationBlocksSynchronousOperation) {
  ...
  EXPECT_TRUE(async_operation_completed_event->Wait(web_rtc::TimeDelta::Zero()));
}
