...
web_rtc::Mutex& GetLoggingLock() {
  static web_rtc::Mutex& mutex = *new web_rtc::Mutex();
  return mutex;
}
...
std::string LogLineRef::DefaultLogLine() const {
  ...
  if (timestamp_ != web_rtc::Timestamp::MinusInfinity()) {
...
LogMessage::LogMessage(const char* file,
  ...
  if (log_timestamp_) {
    ...
    // LogStartTime.
    WallClockStartTime();
    log_line_.set_timestamp(web_rtc::Timestamp::Millis(time));
  }
...
LogMessage::~LogMessage() {
  ...
  web_rtc::MutexLock lock(&GetLoggingLock());
