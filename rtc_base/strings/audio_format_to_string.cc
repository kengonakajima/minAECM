...
namespace rtc {
std::string ToString(const web_rtc::SdpAudioFormat& saf) {
  char sb_buf[1024];
...
std::string ToString(const web_rtc::AudioCodecInfo& aci) {
  char sb_buf[1024];
  rtc::SimpleStringBuilder sb(sb_buf);
...
std::string ToString(const web_rtc::AudioCodecSpec& acs) {
  char sb_buf[1024];
  rtc::SimpleStringBuilder sb(sb_buf);
