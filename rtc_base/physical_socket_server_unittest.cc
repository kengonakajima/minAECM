...
void PhysicalSocketTest::ConnectInternalAcceptError(const IPAddress& loopback) {
  web_rtc::testing::StreamSink sink;
  ...
  EXPECT_FALSE(sink.Check(server.get(), web_rtc::testing::SSE_READ));
  ...
  EXPECT_FALSE(sink.Check(client1.get(), web_rtc::testing::SSE_OPEN));
  EXPECT_FALSE(sink.Check(client1.get(), web_rtc::testing::SSE_CLOSE));

  // Server has pending connection, try to accept it (will fail).
  EXPECT_TRUE_WAIT((sink.Check(server.get(), web_rtc::testing::SSE_READ)),
                   kTimeout);
  ...
  EXPECT_FALSE(sink.Check(client2.get(), web_rtc::testing::SSE_OPEN));
  EXPECT_FALSE(sink.Check(client2.get(), web_rtc::testing::SSE_CLOSE));
