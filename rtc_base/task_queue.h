...
class RTC_LOCKABLE RTC_EXPORT TaskQueue {
 public:
  ...
  using Priority = ::web_rtc::TaskQueueFactory::Priority;

  explicit TaskQueue(std::unique_ptr<web_rtc::TaskQueueBase,
                                     web_rtc::TaskQueueDeleter> task_queue);
  ~TaskQueue();
  ...
  web_rtc::TaskQueueBase* Get() { return impl_; }
 ...
 private:
  web_rtc::TaskQueueBase* const impl_;
};
