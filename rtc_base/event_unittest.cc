...
TEST(EventTest, InitiallySignaled) {
  ...
  ASSERT_TRUE(event.Wait(web_rtc::TimeDelta::Zero()));
}

TEST(EventTest, ManualReset) {
  Event event(true, false);
  ASSERT_FALSE(event.Wait(web_rtc::TimeDelta::Zero()));
 ...
 public:
  void Start(Event* writer, Event* reader) {
    ...
    thread_ = PlatformThread::SpawnJoinable(
        [this] {
          while (!stop_event_.Wait(web_rtc::TimeDelta::Zero())) {
            writer_->Set();
...
TEST(EventTest, UnsignaledWaitDoesNotReturnBeforeTimeout) {
  constexpr web_rtc::TimeDelta kDuration = web_rtc::TimeDelta::Micros(10'499);
  Event event;
  auto begin = web_rtc::Clock::GetRealTimeClock()->CurrentTime();
  EXPECT_FALSE(event.Wait(kDuration));
  EXPECT_GE(web_rtc::Clock::GetRealTimeClock()->CurrentTime(),
            begin + kDuration);
}
...
TEST(EventTest, DISABLED_PerformanceSingleThread) {
  ...
  for (int i = 0; i < kNumIterations; ++i) {
    event.Set();
    event.Wait(web_rtc::TimeDelta::Zero());
  }
}
