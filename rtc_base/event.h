...
class Event {
 public:
  ...
  static constexpr web_rtc::TimeDelta kForever =
      web_rtc::TimeDelta::PlusInfinity();
  ...
  // some other error.
  bool Wait(web_rtc::TimeDelta give_up_after, web_rtc::TimeDelta warn_after);

  // Waits with the given timeout and a reasonable default warning timeout.
  bool Wait(web_rtc::TimeDelta give_up_after) {
    return Wait(give_up_after, give_up_after.IsPlusInfinity()
                                   ? web_rtc::TimeDelta::Seconds(3)
                                   : kForever);
  }
