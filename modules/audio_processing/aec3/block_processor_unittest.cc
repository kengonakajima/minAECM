...
TEST(BlockProcessor, DISABLED_DelayControllerIntegration) {
  ...
  for (auto rate : {16000, 32000, 48000}) {
    ...
    std::unique_ptr<testing::StrictMock<web_rtc::test::MockRenderDelayBuffer>>
        render_delay_buffer_mock(
            new StrictMock<web_rtc::test::MockRenderDelayBuffer>(rate, 1));
  ...
  for (auto rate : {16000, 32000, 48000}) {
    ...
    std::unique_ptr<testing::StrictMock<web_rtc::test::MockRenderDelayBuffer>>
        render_delay_buffer_mock(
            new StrictMock<web_rtc::test::MockRenderDelayBuffer>(rate, 1));
    std::unique_ptr<
        ::testing::StrictMock<web_rtc::test::MockRenderDelayController>>
        render_delay_controller_mock(
            new StrictMock<web_rtc::test::MockRenderDelayController>());
    std::unique_ptr<testing::StrictMock<web_rtc::test::MockEchoRemover>>
        echo_remover_mock(new StrictMock<web_rtc::test::MockEchoRemover>());
...
TEST(BlockProcessor, ExternalDelayAppliedCorrectlyWithInitialCaptureCalls) {
  ...
  std::unique_ptr<testing::NiceMock<web_rtc::test::MockEchoRemover>>
      echo_remover_mock(new NiceMock<web_rtc::test::MockEchoRemover>());
  web_rtc::test::MockEchoRemover* echo_remover_mock_pointer =
