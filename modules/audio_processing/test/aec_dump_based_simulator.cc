...
bool VerifyFixedBitExactness(const web_rtc::audioproc::Stream& msg,
...
bool VerifyFloatBitExactness(const web_rtc::audioproc::Stream& msg,
...
bool ReadNextMessage(bool use_dump_file,
                     ...
                     web_rtc::audioproc::Event& event_msg) {
  if (use_dump_file) {
...
void AecDumpBasedSimulator::PrepareProcessStreamCall(
    const web_rtc::audioproc::Stream& msg) {
...
void AecDumpBasedSimulator::Analyze() {
  ...
  int init_index = 0;
  while (ReadNextMessage(use_dump_file, dump_input_file_, input, event_msg)) {
    if (event_msg.type() == web_rtc::audioproc::Event::INIT) {
      ++init_index;
    ...
    } else if (event_msg.type() == web_rtc::audioproc::Event::STREAM) {
      ++num_capture_frames;
    } else if (event_msg.type() == web_rtc::audioproc::Event::REVERSE_STREAM) {
      ++num_render_frames;
    }
  }
...
void AecDumpBasedSimulator::HandleMessage(
  ...
  if (!settings_.discard_all_settings_in_aecdump) {
    ...
    if (msg.has_aecm_routing_mode() &&
        static_cast<web_rtc::EchoControlMobileImpl::RoutingMode>(
    ...
    if (msg.has_agc_mode() || settings_.agc_mode) {
      ...
      apm_config.gain_controller1.mode =
          static_cast<web_rtc::AudioProcessing::Config::GainController1::Mode>(
              mode);
      if (settings_.use_verbose_logging) {
...
void AecDumpBasedSimulator::HandleMessage(const web_rtc::audioproc::Init& msg,
                                          int init_index) {
...
void AecDumpBasedSimulator::HandleMessage(
    const web_rtc::audioproc::RuntimeSetting& msg) {
  RTC_CHECK(ap_.get());
