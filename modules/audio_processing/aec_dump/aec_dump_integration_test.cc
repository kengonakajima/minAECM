...
namespace {
rtc::scoped_refptr<web_rtc::AudioProcessing> CreateAudioProcessing() {
  rtc::scoped_refptr<web_rtc::AudioProcessing> apm(
      web_rtc::AudioProcessingBuilderForTesting().Create());
  RTC_DCHECK(apm);
  return apm;
}

std::unique_ptr<web_rtc::test::MockAecDump> CreateMockAecDump() {
  auto mock_aec_dump =
      std::make_unique<testing::StrictMock<web_rtc::test::MockAecDump>>();
  ...
  return std::unique_ptr<web_rtc::test::MockAecDump>(std::move(mock_aec_dump));
}
...
TEST(AecDumpIntegration,
  ...
  frame.fill(0.f);
  web_rtc::StreamConfig stream_config(kNumSampleRateHz, kNumChannels);
