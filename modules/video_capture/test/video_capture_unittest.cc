...
using web_rtc::SleepMs;
using web_rtc::VideoCaptureCapability;
using web_rtc::VideoCaptureFactory;
using web_rtc::VideoCaptureModule;
...
class TestVideoCaptureCallback
    : public rtc::VideoSinkInterface<web_rtc::VideoFrame> {
 public:
  TestVideoCaptureCallback()
      : last_render_time_ms_(0),
        incoming_frames_(0),
        timing_warnings_(0),
        rotate_frame_(web_rtc::kVideoRotation_0) {}
  ...
  void OnFrame(const web_rtc::VideoFrame& videoFrame) override {
    web_rtc::MutexLock lock(&capture_lock_);
  ...
  bool CompareLastFrame(const web_rtc::VideoFrame& frame) {
    web_rtc::MutexLock lock(&capture_lock_);
    return web_rtc::test::FrameBufsEqual(last_frame_,
                                        frame.video_frame_buffer());
  }

  void SetExpectedCaptureRotation(web_rtc::VideoRotation rotation) {
    web_rtc::MutexLock lock(&capture_lock_);
    rotate_frame_ = rotation;
  }

 private:
  web_rtc::Mutex capture_lock_;
  VideoCaptureCapability capability_;
  int64_t last_render_time_ms_;
  int incoming_frames_;
  int timing_warnings_;
  rtc::scoped_refptr<web_rtc::VideoFrameBuffer> last_frame_;
  web_rtc::VideoRotation rotate_frame_;
};
...
class VideoCaptureTest : public ::testing::Test {
 public:
  ...
  rtc::scoped_refptr<VideoCaptureModule> OpenVideoCaptureDevice(
      unsigned int device,
      rtc::VideoSinkInterface<web_rtc::VideoFrame>* callback) {
...
TEST_F(VideoCaptureTest, MAYBE_CreateDelete) {
  for (int i = 0; i < 5; ++i) {
    ...
    capability.videoType = web_rtc::VideoType::kUnknown;
#endif
...
TEST_F(VideoCaptureTest, DISABLED_TestTwoCameras) {
  ...
  capability1.videoType = web_rtc::VideoType::kUnknown;
#endif
  ...
  capability2.videoType = web_rtc::VideoType::kUnknown;
#endif
