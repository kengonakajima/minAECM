...
class SharedScreenCastStreamPrivate {
 ...
 private:
  ...
  uint32_t width_ = 0;
  uint32_t height_ = 0;
  web_rtc::Mutex resolution_lock_;
...
void SharedScreenCastStreamPrivate::OnRenegotiateFormat(void* data, uint64_t) {
...

  {
    ...
    web_rtc::MutexLock lock(&that->resolution_lock_);
...
void SharedScreenCastStreamPrivate::UpdateScreenCastStreamResolution(
  ...
  if (width_ != width || height_ != height) {
    ...
    height_ = height;
    {
      web_rtc::MutexLock lock(&resolution_lock_);
...
void SharedScreenCastStreamPrivate::StopAndCleanupStream() {
  ...
  if (pw_stream_) {
    ...
    {
      web_rtc::MutexLock lock(&queue_lock_);
      queue_.Reset();
    }
  }
