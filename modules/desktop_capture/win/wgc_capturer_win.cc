...
bool IsWgcSupported(CaptureType capture_type) {
  ...
  hr = web_rtc::CreateHstring(kApiContract, wcslen(kApiContract), &api_contract);
  if (FAILED(hr))
    return false;
  ...
  web_rtc::DeleteHstring(api_contract);
  if (FAILED(hr) || !is_api_present)
    return false;

  HSTRING wgc_session_type;
  hr = web_rtc::CreateHstring(kWgcSessionType, wcslen(kWgcSessionType),
                             &wgc_session_type);
  if (FAILED(hr))
    return false;
  ...
  web_rtc::DeleteHstring(wgc_session_type);
