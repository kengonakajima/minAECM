...
ForwardErrorCorrection::PacketList MediaPacketGenerator::ConstructMediaPackets(
  ...
  for (int i = 0; i < num_media_packets; ++i) {
    ...
    web_rtc::ByteWriter<uint16_t>::WriteBigEndian(&data[2], seq_num);
    web_rtc::ByteWriter<uint32_t>::WriteBigEndian(&data[4], time_stamp);
    web_rtc::ByteWriter<uint32_t>::WriteBigEndian(&data[8], ssrc_);
