...
int main(int argc, char* argv[]) {
  ...
  rtc::scoped_refptr<web_rtc::AudioMixerImpl> mixer(
      web_rtc::AudioMixerImpl::Create(
          std::unique_ptr<web_rtc::OutputRateCalculator>(
              new web_rtc::DefaultOutputRateCalculator()),
          absl::GetFlag(FLAGS_limiter)));

  const std::vector<std::string> input_files = parse_input_files();
  std::vector<web_rtc::test::FilePlayingSource> sources;
  ...
  web_rtc::WavWriter wav_writer(absl::GetFlag(FLAGS_output_file), sample_rate,
                               num_channels);

  web_rtc::AudioFrame frame;
  ...
  while (!all_streams_finished) {
    ...
    all_streams_finished =
        std::all_of(sources.begin(), sources.end(),
                    [](const web_rtc::test::FilePlayingSource& source) {
