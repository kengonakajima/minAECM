...
TEST(NetEqPerformanceTest, 10_Pl_10_Drift) {
  ...
  int64_t runtime = web_rtc::test::NetEqPerformanceTest::Run(
      web_rtc::field_trial::IsEnabled("WebRTC-QuickPerfTest")
          ? kQuickSimulationTimeMs
          : kSimulationTimeMs,
      kLossPeriod, kDriftFactor);
  ASSERT_GT(runtime, 0);
  web_rtc::test::PrintResult("neteq_performance", "", "10_pl_10_drift", runtime,
                            "ms", true);
}
...
TEST(NetEqPerformanceTest, 0_Pl_0_Drift) {
  ...
  int64_t runtime = web_rtc::test::NetEqPerformanceTest::Run(
      ...
      kLossPeriod, kDriftFactor);
  ASSERT_GT(runtime, 0);
  web_rtc::test::PrintResult("neteq_performance", "", "0_pl_0_drift", runtime,
                            "ms", true);
}
