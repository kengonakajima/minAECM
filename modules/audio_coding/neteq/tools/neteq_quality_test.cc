...
const std::string& DefaultOutFilename() {
  static const std::string path =
      ::web_rtc::test::OutputPath() + "neteq_quality_test_out.pcm";
  return path;
}
...
NetEqQualityTest::NetEqQualityTest(
  ...
  if (out_filename.size() >= 4 &&
    ...
    // Open a wav file.
    output_.reset(
        new web_rtc::test::OutputWavFile(out_filename, 1000 * out_sampling_khz));
  } else {
    // Open a pcm file.
    output_.reset(new web_rtc::test::OutputAudioFile(out_filename));
  }
