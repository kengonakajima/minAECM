...
int main(int argc, char* argv[]) {
  ...
  printf("Input file: %s\n", args[1]);
  std::unique_ptr<web_rtc::test::RtpFileSource> file_source(
      web_rtc::test::RtpFileSource::Create(args[1]));
  ...
  if (absl::GetFlag(FLAGS_audio_level) != -1) {
    ...
    file_source->RegisterRtpHeaderExtension(web_rtc::kRtpExtensionAudioLevel,
  ...
  if (absl::GetFlag(FLAGS_abs_send_time) != -1) {
    ...
    file_source->RegisterRtpHeaderExtension(
        web_rtc::kRtpExtensionAbsoluteSendTime,
  ...
  int cycles = -1;
  std::unique_ptr<web_rtc::test::Packet> packet;
  while (true) {
    ...
    if (packet->header().payloadType == absl::GetFlag(FLAGS_red)) {
      std::list<web_rtc::RTPHeader*> red_headers;
      packet->ExtractRedHeaders(&red_headers);
      while (!red_headers.empty()) {
        web_rtc::RTPHeader* red = red_headers.front();
        RTC_DCHECK(red);
