...
std::vector<float> GetBitrateMultipliers() {
  ...
  const bool use_bitrate_multipliers =
      web_rtc::field_trial::IsEnabled(kBitrateMultipliersName);
  if (use_bitrate_multipliers) {
    const std::string field_trial_string =
        web_rtc::field_trial::FindFullName(kBitrateMultipliersName);
...
AudioEncoderOpusImpl::AudioEncoderOpusImpl(
    ...
    : payload_type_(payload_type),
      send_side_bwe_with_overhead_(
          !web_rtc::field_trial::IsDisabled("WebRTC-SendSideBwe-WithOverhead")),
      use_stable_target_for_adaptation_(!web_rtc::field_trial::IsDisabled(
          "WebRTC-Audio-StableTargetAdaptation")),
      adjust_bandwidth_(
          web_rtc::field_trial::IsEnabled("WebRTC-AdjustOpusBandwidth")),
