...
void OpusTest::Perform() {
  ...
  const std::string file_name_stereo =
      web_rtc::test::ResourcePath("audio_coding/teststereo32kHz", "pcm");
...
void OpusTest::OpenOutFile(int test_number) {
  ...
  file_stream << web_rtc::test::OutputPath() << "opustest_out_" << test_number
              << ".pcm";
  file_name = file_stream.str();
  out_file_.Open(file_name, 48000, "wb");
  file_stream.str("");
  file_name = file_stream.str();
  file_stream << web_rtc::test::OutputPath() << "opusstandalone_out_"
              << test_number << ".pcm";
  file_name = file_stream.str();
