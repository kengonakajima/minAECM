...
void TestWebRtcVadDtx::Test(bool new_outfile, bool expect_dtx_enabled) {
  ...
  out_filename << web_rtc::test::OutputPath() << "testWebRtcVadDtx_outFile_"
               << output_file_num_ << ".pcm";
  Run(web_rtc::test::ResourcePath("audio_coding/testfile32kHz", "pcm"), 32000, 1,
...
void TestOpusDtx::Perform() {
  ...
  std::string out_filename =
      web_rtc::test::OutputPath() + "testOpusDtx_outFile_mono.pcm";
  ...
  out_filename = web_rtc::test::OutputPath() + "testOpusDtx_outFile_stereo.pcm";
  ...
  Run(web_rtc::test::ResourcePath("audio_coding/teststereo32kHz", "pcm"), 32000,
