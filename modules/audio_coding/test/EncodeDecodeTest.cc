...
void Sender::Setup(AudioCodingModule* acm,
  ...
  // Open input file
  const std::string file_name = web_rtc::test::ResourcePath(in_file_name, "pcm");
...
void Receiver::Setup(AudioCodingModule* acm,
  ...
  file_stream << web_rtc::test::OutputPath() << out_file_name << file_num
              << ".pcm";
...
void EncodeDecodeTest::Perform() {
  ...
  for (const auto& send_codec : send_codecs) {
    ...
    std::string fileName = web_rtc::test::TempFilename(
        web_rtc::test::OutputPath(), "encode_decode_rtp");
