...
GetPostProcParamsFromFieldTrialGroup() {
  std::string group = web_rtc::field_trial::FindFullName(
...
LibvpxVp8Decoder::LibvpxVp8Decoder()
    : use_postproc_(
          kIsArm ? web_rtc::field_trial::IsEnabled(kVp8PostProcArmFieldTrial)
                 : true),
...
int LibvpxVp8Decoder::ReturnFrame(
    ...
    uint32_t timestamp,
    int qp,
    const web_rtc::ColorSpace* explicit_color_space) {
  if (img == NULL) {
