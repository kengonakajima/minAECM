...
TEST(Config, NumberOfTemporalLayersIsOne) {
  Config config;
  web_rtc::test::CodecSettings(kVideoCodecH264, &config.codec_settings);
  EXPECT_EQ(1u, config.NumberOfTemporalLayers());
}

TEST(Config, NumberOfTemporalLayers_Vp8) {
  Config config;
  web_rtc::test::CodecSettings(kVideoCodecVP8, &config.codec_settings);
...
TEST(Config, NumberOfTemporalLayers_Vp9) {
  Config config;
  web_rtc::test::CodecSettings(kVideoCodecVP9, &config.codec_settings);
