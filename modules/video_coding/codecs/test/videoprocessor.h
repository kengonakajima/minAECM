...
class VideoProcessor {
 public:
  ...
  VideoProcessor(web_rtc::VideoEncoder* encoder,
 ...
 private:
  class VideoProcessorEncodeCompleteCallback
      : public web_rtc::EncodedImageCallback {
   public:
    ...
    Result OnEncodedImage(
        const web_rtc::EncodedImage& encoded_image,
        const web_rtc::CodecSpecificInfo* codec_specific_info) override {
  ...
  class VideoProcessorDecodeCompleteCallback
      : public web_rtc::DecodedImageCallback {
   public:
    ...
    int32_t Decoded(web_rtc::VideoFrame& image) override;

    int32_t Decoded(web_rtc::VideoFrame& image,
                    int64_t decode_time_ms) override {
      return Decoded(image);
    }

    void Decoded(web_rtc::VideoFrame& image,
  ...
  void FrameEncoded(const web_rtc::EncodedImage& encoded_image,
                    const web_rtc::CodecSpecificInfo& codec_specific);

  // Invoked by the callback adapter when a frame has completed decoding.
  void FrameDecoded(const web_rtc::VideoFrame& image, size_t simulcast_svc_idx);
  ...
  const web_rtc::EncodedImage* BuildAndStoreSuperframe(
  ...
  // Codecs.
  web_rtc::VideoEncoder* const encoder_;
