...
void ConfigureSimulcast(VideoCodec* codec_settings) {
  ...
  const std::vector<web_rtc::VideoStream> streams = cricket::GetSimulcastConfig(
...
void VideoCodecTestFixtureImpl::Config::SetCodecSettings(
  ...
  web_rtc::test::CodecSettings(codec_type, &codec_settings);
...
void VideoCodecTestFixtureImpl::H264KeyframeChecker::CheckEncodedFrame(
    web_rtc::VideoCodecType codec,
    const EncodedImage& encoded_frame) const {
  EXPECT_EQ(kVideoCodecH264, codec);
  bool contains_sps = false;
  bool contains_pps = false;
  bool contains_idr = false;
  const std::vector<web_rtc::H264::NaluIndex> nalu_indices =
      web_rtc::H264::FindNaluIndices(encoded_frame.data(), encoded_frame.size());
  for (const web_rtc::H264::NaluIndex& index : nalu_indices) {
    web_rtc::H264::NaluType nalu_type = web_rtc::H264::ParseNaluType(
        encoded_frame.data()[index.payload_start_offset]);
    if (nalu_type == web_rtc::H264::NaluType::kSps) {
      contains_sps = true;
    } else if (nalu_type == web_rtc::H264::NaluType::kPps) {
      contains_pps = true;
    } else if (nalu_type == web_rtc::H264::NaluType::kIdr) {
      contains_idr = true;
    }
  }
...
VideoCodecTestFixtureImpl::VideoCodecTestFixtureImpl(Config config)
    : encoder_factory_(std::make_unique<web_rtc::VideoEncoderFactoryTemplate<
                           web_rtc::LibvpxVp8EncoderTemplateAdapter,
                           web_rtc::LibvpxVp9EncoderTemplateAdapter,
                           web_rtc::OpenH264EncoderTemplateAdapter,
                           web_rtc::LibaomAv1EncoderTemplateAdapter>>()),
      decoder_factory_(std::make_unique<web_rtc::VideoDecoderFactoryTemplate<
                           web_rtc::LibvpxVp8DecoderTemplateAdapter,
                           web_rtc::LibvpxVp9DecoderTemplateAdapter,
                           web_rtc::OpenH264DecoderTemplateAdapter,
                           web_rtc::Dav1dDecoderTemplateAdapter>>()),
      config_(config) {}
