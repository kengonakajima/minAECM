...
int main(int argc, char* argv[]) {
  std::unique_ptr<web_rtc::test::RtpFileReader> reader;
  ...
  web_rtc::test::RtpPacket packet;
  while (reader->NextPacket(&packet)) {
    web_rtc::RtpPacket header(&rtp_header_extensions);
    header.Parse(packet.data, packet.length);
    uint32_t abs_send_time = 0;
    if (header.GetExtension<web_rtc::AbsoluteSendTime>(&abs_send_time) &&
        abs_send_time != 0)
      ++non_zero_abs_send_time;
    int32_t toffset = 0;
    if (header.GetExtension<web_rtc::TransmissionOffset>(&toffset) &&
        toffset != 0)
      ++non_zero_ts_offsets;
