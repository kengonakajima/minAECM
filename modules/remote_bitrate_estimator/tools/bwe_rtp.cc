...
bool ParseArgsAndSetupRtpReader(
    int argc,
    char** argv,
    std::unique_ptr<web_rtc::test::RtpFileReader>& rtp_reader,
    web_rtc::RtpHeaderExtensionMap& rtp_header_extensions) {
  ...
  if (filename.substr(filename.find_last_of('.')) == ".pcap") {
    ...
    rtp_reader.reset(web_rtc::test::RtpFileReader::Create(
        web_rtc::test::RtpFileReader::kPcap, filename.c_str(), SsrcFilter()));
  } else {
    fprintf(stderr, "Opening as rtp\n");
    rtp_reader.reset(web_rtc::test::RtpFileReader::Create(
        web_rtc::test::RtpFileReader::kRtpDump, filename.c_str()));
  }
  if (!rtp_reader) {
  ...
  web_rtc::RTPExtensionType extension = web_rtc::kRtpExtensionAbsoluteSendTime;
  if (ExtensionType() == "tsoffset") {
    extension = web_rtc::kRtpExtensionTransmissionTimeOffset;
